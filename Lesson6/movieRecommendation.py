# py -m pip install pandas (for installation of pandas library)
...
# Load CSV File
try: df= pd.read_csv("imdb_top_1000.csv") # the variable df i.e dataframe contains the csv file(dataset)

except FileNotFoundError:
    print("File was not found")
    raise SystemExit
genre= sorted({g.strip() for xs in df["Genre"].dropna().str.split(", ")for g in xs}) # this will clean and organize the genre column in the csv file and remove the unwanted things(e.g- remove spaces, dropna will remove missing values from genre column), xs is a temporary variable name, xs represents the genre after splitting the things.
def dots():
    for _ in range(3): print(Fore.YELLOW + ".", end="", flush=True); time.sleep(0.5) #flush to make the things appear immediately and time.sleep() Will delay the time for 0.5 secs, this will make the user think like A.I is thinking.
