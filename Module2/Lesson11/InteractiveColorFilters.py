...
def apply_color_filter(image, filter_type):
    """Apply the specified color filter to the image.""" # This is known as a docstring, string given within triple quotations(""").
    # BGR format: Blue index= 0, Green index= 1, Red index= 2. BGR;s intensity varies from 0 to 255.
...
if image is None:
    ...
    while True:
        ...
        # Wait for key press
        key = cv2.waitKey(0) & 0xFF # This entire arguement will keep the lowest 8 bits. Bits are binary digits(0 and 1), which corresponds to the ASCII key code.
